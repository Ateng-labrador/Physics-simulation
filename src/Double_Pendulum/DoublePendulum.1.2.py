...
def parameter(th1: float, th2: float,
              w1: float, w2: float) -> tuple:
    """
    ...
    a1 = (m2*(
        L1*w1*w1*s*c + L2*w2*w2*s + g*np.sin(th2)*c
        ) - (m1+m2)*g*np.sin(th1)) / D

    a2 = (((m1+m2)*(
        g*(np.sin(th1)*c - np.sin(th2)
           ) - L1*w1*w1*s)) - m2*L2*w2*w2*s*c) / D2
